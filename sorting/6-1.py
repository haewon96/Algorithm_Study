...
for i in range(len(array)):   # 0부터 전체 데이터의 개수(len(array)) - 1까지 반복
    # i : 가장 작은 데이터와 위치가 바뀔 인덱스 (매번 앞쪽으로 보내고자 하는 원소의 위치)
    min_index = i   # 가장 작은 원소의 인덱스 <- 반복할 때마다 매번 가장 작은 원소의 인덱스 고르기
                    # 처음에는, 가장 앞쪽 원소가 가장 작은 원소가 되도록 수행
    for j in range(i + 1, len(array)):   # i + 1부터 전체 원소의 개수(len(array)) - 1까지 선형탐색 수행하여 가장 작은 원소 찾기
        if array[min_index] > array[j]:
            min_index = j   # 현재 가장 작은 원소보다 더 작은 원소가 있다면 그 위치 인덱스 값을 min_index값으로 당김
